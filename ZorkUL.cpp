...
void ZorkUL::createRooms() {
    std::map<string,  std::array<string, 2>> extras{
        {"riddle", std::array<string, 2>{"rich people need it, poor people have it. what is it?", "nothing"}},
        {"key", std::array<string, 2>{"This is the key to open the room where dragon is" ,"take"}},
        {"Hint", std::array<string, 2>{"You need to find the riddle question and then Go find the key for the room where there is a Dragon", "jhhgc"}},
                                       };
    ...
    // Monster mons("dragon",100,50);
    five->addMonster(new Monster("dragonYara",100,50));
...
string ZorkUL::fight(){
    ...
    string output ="\nYou are about to fight the Dragon.\n";


    while (me.getHealth() > 0 && dragon.getHealth() > 0){
        int hitchance = 1 + rand() % 10;


    if (flag){

        dragon.setHealth(me.getHealth() - hitchance);
        flag = false;
}
  else{
